...
# Funkcja obsługująca zaznaczanie obszaru (ROI) w przestrzeni BGR
def select_roi_bgr(frame, roi_number):
    """Pozwala użytkownikowi wybrać region na obrazie BGR."""
...
# Lista do przechowywania zakresów HSV
hsv_ranges = []

# Zbieranie zakresów dla 3 kolorów
for i in range(1, 4):
    # Przechwycenie nowej klatki z kamery
    frame = picam2.capture_array()  # Dane są w formacie BGR
    cv2.imshow("Camera Feed", frame)  # Wyświetlenie w BGR
    cv2.waitKey(1)
