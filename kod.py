...
def toggle_motor():
    ...
    global motor_running, dc_motor_enable 
    if dc_motor_enable:
        # włącz silnik dc
        in1.value = 0  
        in2.value = 0
    ...
    dc_motor_enable = not dc_motor_enable
...
def detect_and_track_objects(frame, mask, color, target_x=320):
    global last_stop_time
    global motor_running
    global motor_stop_delay
    global belt_running
    global dc_motor_enable
    ...
    if  not object_visible and (time.time() - last_stop_time) >= motor_stop_delay and dc_motor_enable:
...
while True:
    ...
    cv2.imshow("Obraz z kamery", frame_with_contours)

    # Wyświetlanie wykrytego koloru
    if detected_color:
        print(f"Wykryto: {detected_color}")
